...
import MessageScheduleCalculation from "./MessageScheduleCalculation";
import CompressionCalculation from "./CompressionCalculation";
import ButtonBack from "./ButtonBack";
import ButtonBackFast from "./ButtonBackFast";
import ButtonInit from "./ButtonBackInit";
import ButtonClock from "./ButtonClock";
import ButtonClockFast from "./ButtonClockFast";
import ButtonClockFinish from "./ButtonClockFinish";
import ButtonAutoClock from "./ButtonAutoClock";
import BeforeLetters from "./BeforeLetters";
...
const SHA = () => {
  ...
  function onInputChange(value) {
    if (inputBase === "bin" && !["0", "1", ""].includes(value.substr(-1)))
      return;
    if (
      inputBase === "hex" &&
      ![
  ...
  return (
    <div
      className="App font-mono text-xs text-indigo-300 px-2 leading-3 bg-gray-900 h-screen transition"
      style={{  
        height : "130%" ,
    ...
    >
      <div className="flex flex-col lg:flex-row w-full mb-2">
        <div className="w-full SEARCH-BAR">
          <div className="flex">
            <select
              value={inputBase}
              onChange={(e) => onInputBaseChange(e.target.value)}
              className="bg-gray-700 mt-2 py-2 px-3 rounded mr-2 cursor-pointer hover:bg-gray-600 transition"
            >
            ...
            <input
              type="text"
              className="bg-gray-700 mt-2 w-full py-2 px-3 mr-2 rounded"
              value={input}
            ...
            <select
              ...
              className="bg-gray-700 my-2 py-2 px-3 rounded mr-2 cursor-pointer hover:bg-gray-600 transition hidden"
            >
              <option value="hex">Hex</option>
              <option value="bin">Bin</option>
            </select>
        ...
        <div className="w-full mt-2 items-center flex BUTTONS">
          ...
          <ButtonAutoClock
            onAutoClock={onAutoClock}
            clock={clock}
            lastClock={lastClock}
            autoplay={autoplay}
          />
          <ButtonClockFast
            onClockFast={onClockFast}
            clock={clock}
            lastClock={lastClock}
          />
          <ButtonClockFinish
            onClockFinish={onClockFinish}
            clock={clock}
            lastClock={lastClock}
          />
        </div>
      ...
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="col pr-1 w-[290px] FIRST-DIV ">
          <MessageBlock
            data={paddedInput}
            base={base}
            clock={cycleClock()}
            chunksLoop={chunksLoop(clock)}
          />
          <Explainer
            clock={cycleClock()}
            masterClock={clock}
            ...
            lastClock={lastClock(clock)}
          />
        </div>
        <div className="col pr- SECOND-DIV">
          <MessageSchedule
            data={wView}
            base={base}
            labels={"w"}
            clock={cycleClock()}
            currentChunk={chunksLoop(clock)}
          />
        </div>
        <div className="col pr-4 SECOND-LAST-DIV">
          <div className="pr-4">
            <div>
              <BeforeLetters
        ...
        <div className="col  RIGHT-MOST-DIV">
